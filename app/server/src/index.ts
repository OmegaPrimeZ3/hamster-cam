...
export async function buildServer(): Promise<AppServer> {
  ...
  registerPrivateMedia(app, 'thumbnails', new Set(['.jpg', '.jpeg']));
...
// Content-type table for all private-media routes.
...
/**
 * Stream `STORAGE_PATH/<subdir>/<file>` for `GET /<subdir>/*` after a session
 * check. Path-traversal and extension-allowlist guards apply. For `.mp4` files
 * (and any caller that passes `supportsRange: true`) the handler honours HTTP
 * Range requests (206 partial) so iOS Safari `<video>` playback works without
 * buffering the whole clip first.
 */
function registerPrivateMedia(
  ...
  app.get(`/${subdir}/*`, async (req, reply) => {
    // Inline auth check — keeps the route off the `preHandler` typing path.
    ...
    if (!st.isFile() || st.size === 0) {
      // Zero-byte placeholders (Frigate unreachable at capture time) → 404.
      return reply.code(404).send({ error: 'not found' });
    }
    ...
    // HTTP Range support — mandatory for iOS Safari <video> playback.
    if (rangeHeader) {
