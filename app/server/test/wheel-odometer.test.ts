...
// ---------------------------------------------------------------------------
// Coalesce grace window — beginWheelCoalesce / cancelWheelCoalesce /
// isWheelCoalescing behaviour.
// ---------------------------------------------------------------------------

describe('wheel coalesce grace window', () => {
  it('beginWheelCoalesce keeps session alive and fires onFinalEnd after WHEEL_SESSION_COALESCE_MS', async () => {
    vi.useFakeTimers();
    const camId = await seedWheelCamera();
    const { startWheelSession, beginWheelCoalesce, _activeSessions, _coalescingSessions } =
      await import('../src/wheel-odometer.js');

    startWheelSession(camId, Date.now());
    expect(_activeSessions.size).toBe(1);

    const endedWith: number[] = [];
    beginWheelCoalesce(camId, Date.now(), (metres) => {
      endedWith.push(metres);
    });

    // Session is still alive during the grace window.
    expect(_activeSessions.size).toBe(1);
    ...
    expect(endedWith).toHaveLength(1);

    vi.useRealTimers();
  });

  it('cancelWheelCoalesce stops the timer and session continues running', async () => {
    vi.useFakeTimers();
    const camId = await seedWheelCamera();
    const {
      startWheelSession,
    ...
    // Zone re-entry — cancel the coalesce.
    const cancelled = cancelWheelCoalesce(camId);
    expect(cancelled).toBe(true);
    expect(_coalescingSessions.size).toBe(0);

    // Session is still alive (ffmpeg kept running).
    expect(_activeSessions.size).toBe(1);

    // Advance past what would have been the coalesce deadline — onFinalEnd must NOT fire.
    await vi.advanceTimersByTimeAsync(15_000);
    await Promise.resolve();
    expect(endedWith).toHaveLength(0); // no spurious callback

    vi.useRealTimers();
  });

  it('zone re-entry during coalesce continues accumulating rotations without reset', async () => {
    vi.useFakeTimers();
    const camId = await seedWheelCamera();
    const {
      startWheelSession,
    ...
    // Feed one rotation before zone-exit.
    const header = Buffer.from('P5\n4 4\n255\n', 'ascii');
    const light = Buffer.alloc(16, 200);
    const dark = Buffer.alloc(16, 50);
    session?.proc.stdout.emit('data', Buffer.concat([header, light]));
    session?.proc.stdout.emit('data', Buffer.concat([header, dark]));
    session?.proc.stdout.emit('data', Buffer.concat([header, light]));

    expect(session?.counter.getRotations()).toBe(1);

    // Zone-exit: coalesce starts. Session and counter stay alive.
    const endedMetres: number[] = [];
    beginWheelCoalesce(camId, Date.now(), (m) => endedMetres.push(m));

    // Feed another rotation DURING the coalesce window (wheel still spinning).
    for (let i = 0; i < 6; i += 1) {
      session?.proc.stdout.emit('data', Buffer.concat([header, light]));
    }
    session?.proc.stdout.emit('data', Buffer.concat([header, dark]));
    ...
    expect(session?.counter.getRotations()).toBe(2);

    // Zone re-entry: cancel coalesce.
    cancelWheelCoalesce(camId);

    // Feed a third rotation after re-entry.
    for (let i = 0; i < 6; i += 1) {
      session?.proc.stdout.emit('data', Buffer.concat([header, light]));
    }
    session?.proc.stdout.emit('data', Buffer.concat([header, dark]));
    ...
    expect(session?.counter.getRotations()).toBe(3);

    // Session finally ends — 3 rotations accumulated across coalesced fragments.
    const metres = endWheelSession(camId);
    expect(metres).not.toBeNull();
    // 3 × π × 152 / 1000
    expect(metres as number).toBeCloseTo(3 * Math.PI * 152 / 1000, 4);

    vi.useRealTimers();
  });

  it('back-to-back exits (no re-entry between them) increment coalescedExits', async () => {
    // When beginWheelCoalesce is called while a coalesce is already active
    // (e.g. zone-exit, coalesce armed, then ANOTHER zone-exit on the same camera
    // before a re-entry), the existing handle is replaced and coalescedExits
    // increments. This represents Frigate rapidly cycling exit→exit on the same
    // object, which is unusual but should be handled gracefully.
    vi.useFakeTimers();
    const camId = await seedWheelCamera();
    const { startWheelSession, beginWheelCoalesce, _coalescingSessions } =
      await import('../src/wheel-odometer.js');

    startWheelSession(camId, Date.now());

    // First exit — coalescedExits = 1.
    const calls: number[] = [];
    beginWheelCoalesce(camId, 100, (m) => calls.push(m));
    expect(_coalescingSessions.get(camId)?.coalescedExits).toBe(1);

    // Second exit without re-entry — replaces the timer, coalescedExits = 2.
    beginWheelCoalesce(camId, 200, (m) => calls.push(m));
    expect(_coalescingSessions.get(camId)?.coalescedExits).toBe(2);

    // Third exit without re-entry — coalescedExits = 3.
    let finalZoneExitAt = 0;
    beginWheelCoalesce(camId, 12345, (_m, zoneExitAt) => {
      calls.push(_m);
      finalZoneExitAt = zoneExitAt;
    });
    expect(_coalescingSessions.get(camId)?.coalescedExits).toBe(3);

    // Timer fires — exactly one final callback.
    await vi.advanceTimersByTimeAsync(11_000);
    await Promise.resolve();

    expect(calls).toHaveLength(1);
    // zoneExitAt reflects the LAST zone-exit (the one that armed the current timer).
    expect(finalZoneExitAt).toBe(12345);

    vi.useRealTimers();
  });

  it('wheel_mark_enabled=false: no coalesce ever starts because no session starts', async () => {
    const camId = await seedWheelCamera(false); // wheel_mark_enabled = false
    const { startWheelSession, _activeSessions, _coalescingSessions } =
      await import('../src/wheel-odometer.js');
  ...
  it('zoneExitAt in callback matches the original zone-exit time, not the timer expiry', async () => {
    vi.useFakeTimers();
    const camId = await seedWheelCamera();
    const { startWheelSession, beginWheelCoalesce } = await import('../src/wheel-odometer.js');

    startWheelSession(camId, Date.now());

    const originalExitAt = 9_999_888_777;
    let receivedAt = -1;
    beginWheelCoalesce(camId, originalExitAt, (_m, at) => {
      receivedAt = at;
    });

    // Advance past coalesce window — timer fires much later than originalExitAt.
    await vi.advanceTimersByTimeAsync(11_000);
    await Promise.resolve();

    // zoneExitAt must be the original exit time, not Date.now() at callback time.
    expect(receivedAt).toBe(originalExitAt);
